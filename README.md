...
PDFs are not generated by CI, so re-export them whenever the matching page
changes. Both were produced with Chromium's *Print → Save as PDF*, A4:

- `1c-rabbitmq-plan.pdf` — printed from `plan.html` (it has a print stylesheet);
- `1c-rabbitmq-presentation.pdf` — its print source is not in this repository.
